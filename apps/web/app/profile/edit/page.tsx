...
const EditProfilePage: React.FC = () => {
  ...
  const { data: locationData, isLoading: isLoadingLocation } =
    trpc.location.getMyLocation.useQuery(undefined, {
      enabled: status === "authenticated",
    });
  ...
  const handleSubmit = useCallback(
    (e: React.FormEvent) => {
      ...
      const newRadiusNumber =
        serviceRadius === "" ? undefined : Number(serviceRadius);
      // Handle comparison carefully: compare numbers or both undefined
      const currentRadiusNumber =
        currentRadius === "" ? undefined : Number(currentRadius);
      if (newRadiusNumber !== currentRadiusNumber) {
        // Handle potential NaN if input is invalid, send undefined if empty or NaN
        const radiusValue =
          serviceRadius === "" || isNaN(newRadiusNumber as number)
            ? undefined
            : newRadiusNumber;
      ...
      // Check skills change
      const currentSkillsString =
        currentProfile.skills?.map((s) => s.name).join(", ") || "";
    ...
    [
      ...
      image,
      serviceRadius,
      skillsInput, // Include all state values used
      profileData,
        ...
        <div className="max-w-2xl mx-auto bg-white p-8 border-2 border-black rounded-lg shadow-[8px_8px_0px_0px_rgba(0,0,0,1)]">
          ...
          <form onSubmit={handleSubmit} className="space-y-6">
            ...
            <div>
              <label
                htmlFor="name"
                className="block text-base font-bold font-satoshi text-black mb-1"
              >
                Name
              </label>
              <input
            ...
            {/* Location & Radius */}
            <div>
              <label className="block text-base font-bold font-satoshi text-black mb-1">
                Location
              </label>
              <p className="text-xs text-gray-500 mb-3">
                Set your precise location to help others find your services
                nearby.
              </p>
              <LocationPicker
                standalone={true}
            ...
            <div>
              ...
              <p className="text-xs text-gray-500 mt-1">
                How far are you willing to travel for services? (in kilometers)
              </p>
            </div>
              ...
              <button
                ...
                className={`w-full flex justify-center py-2.5 px-4 border-2 border-black rounded-lg font-bold text-base text-white bg-blue-600 hover:bg-blue-700 focus:outline-none focus-visible:ring-2 focus-visible:ring-offset-2 focus-visible:ring-blue-500 shadow-[4px_4px_0px_0px_rgba(0,0,0,1)] hover:translate-x-[2px] hover:translate-y-[2px] hover:shadow-[2px_2px_0px_0px_rgba(0,0,0,1)] transition-all duration-150 ${
                  updateProfileMutation.isPending
                    ? "bg-gray-400 hover:bg-gray-400 cursor-not-allowed hover:shadow-[4px_4px_0px_0px_rgba(0,0,0,1)] hover:translate-x-0 hover:translate-y-0"
                    : ""
                }`}
              >
                {updateProfileMutation.isPending ? (
