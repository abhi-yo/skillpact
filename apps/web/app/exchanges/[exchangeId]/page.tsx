...
const ExchangeDetailPage: React.FC = () => {
  ...
  const {
    ...
    error,
    refetch,
  } = trpc.exchange.getExchangeById.useQuery(
    { id: exchangeId },
    {
  ...
  const respondMutation = trpc.exchange.respondToRequest.useMutation({
    ...
    onSuccess: (data) => {
      toast.success(
        `Exchange ${data.status === "ACCEPTED" ? "accepted" : "declined"}.`
      );
  ...
  const handleComplete = () => {
    ...
    const hoursInput = prompt(
      "Enter hours completed (optional, min 0.5):",
      "1"
    );
    ...
    if (hoursInput) {
      ...
      if (!isNaN(parsedHours) && parsedHours >= 0.5) {
        hours = parsedHours;
      } else if (hoursInput.trim() !== "") {
        // Allow empty input to proceed without hours
        toast.error(
          "Invalid hours entered. Please enter a number >= 0.5 or leave blank."
        );
    ...
    if (
      window.confirm("Are you sure you want to mark this exchange as complete?")
    ) {
  ...
  const handleCancel = () => {
    ...
    if (
      window.confirm(
        "Are you sure you want to cancel this exchange? This cannot be undone."
      )
    ) {
  ...
  const isCancellable =
    exchange &&
    ["REQUESTED", "ACCEPTED", "SCHEDULED"].includes(exchange.status);
      ...
      <DashboardLayout>
        <div className="container mx-auto p-6 text-center">
          <p className="text-gray-600">
            Exchange not found or still loading...
          </p>
          <Link href="/exchanges">
  ...
  const canChat =
    (exchange.status === "ACCEPTED" || exchange.status === "SCHEDULED") &&
          ...
          <Card className="max-w-md mx-auto border-2 border-black rounded-lg shadow-[6px_6px_0px_0px_rgba(0,0,0,1)] bg-white">
            <CardHeader className="bg-red-100 border-b-2 border-black rounded-t-lg">
              <CardTitle className="flex items-center justify-center text-red-700">
                <AlertCircle className="mr-2" size={20} /> Error Loading
                Exchange
              </CardTitle>
            </CardHeader>
  ...
  const nbButtonBase =
    "w-full text-sm font-bold border-2 border-black rounded-lg shadow-[4px_4px_0px_0px_rgba(0,0,0,1)] hover:translate-x-[1px] hover:translate-y-[1px] hover:shadow-none transition-all disabled:opacity-70 disabled:shadow-none disabled:translate-x-0 disabled:translate-y-0 flex items-center justify-center py-2 px-4";
            ...
            <div className="lg:col-span-2">
              <div className="bg-white border-2 border-black rounded-lg shadow-[6px_6px_0px_0px_rgba(0,0,0,1)]">
                ...
                <div className="px-6 py-4 border-b-2 border-black bg-gray-100 rounded-t-lg flex justify-between items-center">
                  ...
                  <div
                    className={`flex items-center px-2 py-1 border rounded-lg ${statusColor} text-xs font-bold`}
                  >
                ...
                <div className="px-6 py-6 space-y-6">
                  ...
                  <div className="flex items-center justify-between space-x-4">
                    ...
                    <div className="flex-1 p-4 border-2 border-black rounded-lg bg-gray-50 shadow-[4px_4px_0px_0px_rgba(0,0,0,0.1)]">
                      ...
                      <div className="flex items-center space-x-3">
                        <Image
                          src={
                            exchange.requester?.image || "/default-avatar.png"
                    ...
                    <div className="flex-1 p-4 border-2 border-black rounded-lg bg-gray-50 shadow-[4px_4px_0px_0px_rgba(0,0,0,0.1)]">
                      ...
                      <div className="flex items-center space-x-3">
                        <Image
                          src={
                            exchange.provider?.image || "/default-avatar.png"
                          } // Provide a fallback image
                  ...
                  <div className="p-4 border-2 border-black rounded-lg bg-yellow-50 shadow-[4px_4px_0px_0px_rgba(0,0,0,0.1)]">
                    ...
                    <Link
                      href={`/services/${exchange.providerServiceId}`}
                    ...
                    {exchange.requesterService && (
                      <p className="text-sm mt-1">
                        Offered in return: {exchange.requesterService.title}
                      </p>
                    )}
                  </div>
                ...
                <div className="px-6 py-6 space-y-4">
                    ...
                    <>
                      {isProvider && (
                        <>
                          <p className="text-sm font-medium mb-3">
                            Respond to request from {exchange.requester?.name}:
                          </p>
                      ...
                      {isRequester && (
                        <p className="text-sm text-gray-600 italic">
                          Waiting for {exchange.provider?.name} to respond.
                        </p>
                    ...
                    <>
                      ...
                      >
                        <DialogTrigger asChild>
                          <Button
                            disabled={isLoadingAction}
                            className="w-full bg-indigo-200 border-2 border-black rounded-lg shadow-[4px_4px_0px_0px_rgba(0,0,0,1)] hover:shadow-none hover:translate-x-[2px] hover:translate-y-[2px] transition-all"
                          >
                            <CalendarIcon size={16} className="mr-2" /> Set
                            Schedule
                          </Button>
                        </DialogTrigger>
                        <DialogContent className="sm:max-w-[425px] bg-white border-2 border-black rounded-lg shadow-[6px_6px_0px_0px_rgba(0,0,0,1)]">
                          <DialogHeader className="border-b-2 border-black pb-3">
                            <DialogTitle className="font-satoshi text-lg font-bold">
                              Schedule Exchange
                            </DialogTitle>
                            <DialogDescription>
                          ...
                          <DialogFooter className="border-t-2 border-black pt-4">
                            <DialogClose asChild>
                              <Button
                                type="button"
                                className="bg-gray-200 border-2 border-black rounded-lg shadow-[4px_4px_0px_0px_rgba(0,0,0,1)] hover:shadow-none hover:translate-x-[2px] hover:translate-y-[2px] transition-all"
                              >
                                Cancel
                              </Button>
                            </DialogClose>
                            <Button
                              ...
                              disabled={
                                !selectedScheduleDate || isLoadingAction
                              }
                  ...
                  {exchange.status === "SCHEDULED" && (
                    <>
                      ...
                      {isRequester && (
                        <p className="text-sm text-gray-600 italic">
                          Waiting for {exchange.provider?.name} to mark as
                          complete after the service.
                        </p>
                      )}
                  ...
                  {exchange.status === "COMPLETED" && (
                    <div className="mt-6 p-4 border-2 border-black rounded-lg bg-blue-50">
                      ...
                      <p className="text-sm mb-4">
                        Share your feedback about your experience with{" "}
                        {exchange.provider?.name}. Your rating helps build
                        community trust.
                      </p>
                      ...
                      (isRequester && exchange.requesterRating === null) ? (
                        <Link
                          href={`/exchanges/${exchange.id}/rate`}
                          className="inline-flex items-center px-4 py-2 bg-blue-600 text-white font-bold border-2 border-black rounded-lg shadow-[4px_4px_0px_0px_rgba(0,0,0,1)] hover:translate-x-[2px] hover:translate-y-[2px] hover:shadow-none transition-all"
                        >
