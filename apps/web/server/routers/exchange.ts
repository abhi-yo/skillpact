...
import { ExchangeStatus, NotificationType, Prisma } from "database";
...
export const exchangeRouter = router({
  ...
  getActiveExchangeSummary: protectedProcedure.query(async ({ ctx }) => {
    const userId = ctx.session.user.id;
  ...
  requestExchange: protectedProcedure
    ...
    .mutation(async ({ ctx, input }) => {
      ...
      if ((service.hourlyRate ?? 0) > requester.credits) {
        throw new TRPCError({
          code: "BAD_REQUEST",
          message: "You do not have enough credits to request this service.",
        });
      }
  ...
  scheduleExchange: protectedProcedure
    ...
    .mutation(async ({ ctx, input }) => {
      ...
      if (
        ...
        exchange.status !== EXCHANGE_STATUS.SCHEDULED
      ) {
        throw new TRPCError({
          code: "BAD_REQUEST",
  ...
  getUserExchanges: protectedProcedure
    .input(
      z.object({
        status: z
          .enum(["all", "pending", "upcoming", "completed", "cancelled"])
          .optional()
          .default("all"),
      })
    )
      ...
      switch (input.status) {
        ...
        case "upcoming":
          statusFilter = {
            OR: [{ providerId: userId }, { requesterId: userId }],
            status: {
              in: [EXCHANGE_STATUS.ACCEPTED, EXCHANGE_STATUS.SCHEDULED],
            },
          };
          break;
        case "completed":
        ...
        case "cancelled":
          statusFilter = {
            OR: [{ providerId: userId }, { requesterId: userId }],
            status: {
              in: [EXCHANGE_STATUS.CANCELLED, EXCHANGE_STATUS.DECLINED],
            },
          };
          break;
        default:
  ...
  cancelExchange: protectedProcedure
    ...
    .mutation(async ({ ctx, input }) => {
      ...
      const cancellableStatuses: ExchangeStatus[] = [
        "REQUESTED",
        "ACCEPTED",
        "SCHEDULED",
      ];
