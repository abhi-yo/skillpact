...
export const serviceRouter = router({
  ...
  getNearbyServices: protectedProcedure
    ...
    .query(async ({ ctx, input }) => {
      ...
      try {
        // Get the current user's coordinates & radius
        ...
        if (
          ...
          throw new TRPCError({
            ...
            message:
              "Please set your location and service radius in your profile to see nearby services.",
          });
        }
        ...
        if (Math.abs(lat) > 90 || Math.abs(lng) > 180 || radius <= 0) {
          throw new TRPCError({
            code: "BAD_REQUEST",
            message: "Invalid location data in your profile",
          });
        }
    ...
    .mutation(async ({ ctx, input }) => {
      ...
      if (activeExchanges > 0) {
        throw new TRPCError({
          code: "CONFLICT",
          message:
            "This service cannot be deleted because it is involved in active exchanges. Please complete or cancel them first.",
        });
      }
