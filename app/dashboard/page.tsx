...
import {
  User, BarChart, Activity, PlusCircle, Search, ArrowRightLeft,
...
const DashboardPage: React.FC = () => {
  ...
  // --- tRPC Queries ---
  const { data: userProfile, isLoading: profileLoading, error: profileError } = trpc.user.getProfile.useQuery(undefined, { enabled: !!userId && isSignedIn });
  const { data: profileCompletionData } = trpc.user.getProfileCompletion.useQuery(undefined, { enabled: !!userId && isSignedIn });
  const utils = trpc.useContext();
  const { data: dashboardStats } = trpc.user.getDashboardStats.useQuery(undefined, { enabled: !!userId && isSignedIn, staleTime: 0, refetchOnMount: true, refetchOnWindowFocus: true });
  const { data: pendingExchanges } = trpc.exchange.getPendingExchanges.useQuery(undefined, { enabled: !!userId && isSignedIn });
  const { data: upcomingExchanges } = trpc.exchange.getUpcomingExchanges.useQuery(undefined, { enabled: !!userId && isSignedIn });
  const { data: recentActivity } = trpc.exchange.getRecentActivity.useQuery(undefined, { enabled: !!userId && isSignedIn, staleTime: 0, refetchOnMount: true, refetchOnWindowFocus: true });
  ...
  const { profileCompletion, hasMissingRadius, hasMissingSkills, hasMissingServices } = useMemo(() => {
    ...
    return { profileCompletion: 0, hasMissingRadius: true, hasMissingSkills: true, hasMissingServices: true };
  ...
  if (isLoaded && isSignedIn) {
    ...
    if (totalVisibleActive === 1) {
      if (pendingExchangesCount === 1) singleVisibleActiveId = pendingExchanges?.[0]?.id;
      else if (upcomingExchangesCount === 1) singleVisibleActiveId = upcomingExchanges?.[0]?.id;
    }
            ...
            <header className="w-full bg-white/80 backdrop-blur">
              <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8  mt-2">
                <div className="flex flex-col md:flex-row justify-between items-start md:items-center">
                    <h1 className="font-satoshi tracking-tight text-2xl sm:text-3xl font-bold mb-1 md:mb-0 pr-12 sm:pr-0">
                    Welcome, {(session?.user?.name || 'User').split(' ')[0]}
                    </h1>
                    <div className="flex items-center space-x-4">
                    <Link href="/notifications" className="relative p-1.5 sm:p-2 bg-white border-2 border-black shadow-[2px_2px_0px_0px_rgba(0,0,0,1)] hover:translate-x-[1px] hover:translate-y-[1px] hover:shadow-none transition-all focus-visible:ring-2 focus-visible:ring-offset-2 focus-visible:ring-black rounded-lg">
                    ...
                    <button
                        onClick={() => signOut({ callbackUrl: '/' })}
                        className="px-3 py-1.5 sm:px-4 sm:py-2 bg-red-100 text-red-700 font-bold border-2 border-black shadow-[4px_4px_0px_0px_rgba(0,0,0,1)] hover:translate-x-[2px] hover:translate-y-[2px] hover:shadow-none transition-all text-sm sm:text-base focus-visible:ring-2 focus-visible:ring-offset-2 focus-visible:ring-black rounded-lg"
                    >
                        Sign Out
                    </button>
                    </div>
                ...
                <div className="grid grid-cols-1 lg:grid-cols-9 gap-6">
                    ...
                    <div className="lg:col-span-6 space-y-8">
                        ...
                        <div className="bg-white border-2 border-black shadow-[6px_6px_0px_0px_rgba(0,0,0,1)] p-6 rounded-lg">
                            <h2 className="font-satoshi tracking-tight text-2xl font-bold text-black mb-5">Quick Actions</h2>
                            <div className="grid grid-cols-2 md:grid-cols-4 gap-5 font-inter">
                            <Link href="/services/new" className="block p-3 bg-white border-2 border-black shadow-[4px_4px_0px_0px_rgba(0,0,0,1)] hover:translate-x-[2px] hover:translate-y-[2px] hover:shadow-none transition-all text-center focus-visible:ring-2 focus-visible:ring-offset-2 focus-visible:ring-black rounded-lg">
                                <PlusCircle className="mx-auto mb-1 text-green-600" size={24} strokeWidth={2} />
                                <span className="font-bold text-base text-black">Offer Service</span>
                            </Link>
                            <Link href="/services/browse" className="block p-3 bg-white border-2 border-black shadow-[4px_4px_0px_0px_rgba(0,0,0,1)] hover:translate-x-[2px] hover:translate-y-[2px] hover:shadow-none transition-all text-center focus-visible:ring-2 focus-visible:ring-offset-2 focus-visible:ring-black rounded-lg">
                                <Search className="mx-auto mb-1 text-blue-600" size={24} strokeWidth={2} />
                                <span className="font-bold text-base text-black">Find Services</span>
                            </Link>
                            <Link 
                            ...
                            >
                                <ArrowRightLeft className="mx-auto mb-1 text-purple-600" size={24} strokeWidth={2} />
                                <span className="font-bold text-base text-black">My Exchanges</span>
                            </Link>
                            <Link href="/services/my-services" className="block p-3 bg-white border-2 border-black shadow-[4px_4px_0px_0px_rgba(0,0,0,1)] hover:translate-x-[2px] hover:translate-y-[2px] hover:shadow-none transition-all text-center focus-visible:ring-2 focus-visible:ring-offset-2 focus-visible:ring-black rounded-lg">
                                <List className="mx-auto mb-1 text-yellow-600" size={24} strokeWidth={2} />
                                <span className="font-bold text-base text-black">My Services</span>
                            </Link>
                            </div>
                        </div>
                        ...
                        <div className="bg-white border-2 border-black shadow-[6px_6px_0px_0px_rgba(0,0,0,1)] rounded-lg">
                            ...
                            <h2 className="font-satoshi tracking-tight text-lg md:text-2xl font-bold text-white flex items-center">
                                <BarChart className="mr-2 md:mr-3" size={28} strokeWidth={2} /> 
                            ...
                            <div className="flex items-center gap-2 md:gap-3 w-full sm:w-auto">
                                ...
                                <Link href="/activity" className="text-sm md:text-base font-bold text-white bg-blue-400 px-3 py-1.5 md:px-4 md:py-2 border-2 border-white shadow-[2px_2px_0px_0px_rgba(255,255,255,0.8)] hover:translate-x-[1px] hover:translate-y-[1px] hover:shadow-none transition-all duration-150 flex items-center justify-center focus-visible:ring-2 focus-visible:ring-offset-1 focus-visible:ring-white flex-1 sm:flex-none rounded-lg">
                                <span className="hidden sm:inline">View Detailed Activity</span>
                                <span className="sm:hidden">View Details</span>
                                <ChevronRight className="h-3 w-3 md:h-4 md:w-4 ml-1" strokeWidth={3}/>
                                </Link>
                            </div>
                            </div>
                            <div className="px-6 py-6 font-inter">
                                <div className="flex flex-col md:flex-row items-center justify-center md:justify-around gap-6 md:gap-8 mb-10">
                                    <div className="flex items-center gap-4 bg-blue-100 p-4 border border-black rounded-lg"> 
                                        <div className="relative w-32 h-32 flex-shrink-0">
                                            ...
                                            <div className="absolute inset-0 bg-green-200" style={{ clipPath: servicesOfferedCount > 0 ? `polygon(0 0, ${Math.min(100, (servicesOfferedCount / (servicesOfferedCount + servicesReceivedCount) || 0) * 100)}% 0, ${Math.min(100, (servicesOfferedCount / (servicesOfferedCount + servicesReceivedCount) || 0) * 100)}% 100%, 0 100%)` : 'none' }}></div>
                                            <div className="absolute inset-0 bg-blue-200" style={{ clipPath: servicesReceivedCount > 0 ? `polygon(${Math.max(0, (servicesOfferedCount / (servicesOfferedCount + servicesReceivedCount) || 0) * 100)}% 0, 100% 0, 100% 100%, ${Math.max(0, (servicesOfferedCount / (servicesOfferedCount + servicesReceivedCount) || 0) * 100)}% 100%)` : 'none' }}></div>
                                        ...
                                        <div className="flex flex-col space-y-1">
                                            ...
                                            <span className="text-base ml-1.5">Offered: {servicesOfferedCount}</span>
                                            </div>
                                            <div className="flex items-center">
                                            <div className="w-3 h-3 bg-blue-200 border border-black"></div>
                                            <span className="text-base ml-1.5">Received: {servicesReceivedCount}</span>
                                            </div>
                                        </div>
                                    </div>
                                    <div className="bg-blue-100 px-6 py-5 border-2 border-black shadow-[4px_4px_0px_0px_rgba(0,0,0,1)] min-w-[160px] rounded-lg">
                                        <div className="flex flex-col items-center justify-center">
                                            <span className="font-satoshi text-base font-semibold text-black mb-1">Hours Banked</span>
                                            <span className="text-2xl font-bold text-black">{hoursBanked}</span>
                                ...
                                <div className="border-t-2 border-black pt-4 mb-6 px-6">
                                    <div className="flex justify-between items-center mb-3">
                                        <h3 className="font-satoshi text-xl font-bold text-black">Pending Exchanges</h3>
                                    ...
                                    <div className="space-y-3">
                                        ...
                                        <div key={exchange.id} className="mb-3 p-3 border-2 border-black bg-yellow-50 rounded-lg">
                                            <div className="flex justify-between">
                                                ...
                                                <span className="bg-yellow-200 text-sm px-2 py-1 border border-black">Action needed</span>
                                            </div>
                                            ...
                                            <Link href={`/exchanges/${exchange.id}`} className="px-4 py-2 text-base bg-blue-200 border border-black hover:bg-blue-300 transition-colors focus-visible:ring-1 focus-visible:ring-offset-1 focus-visible:ring-black">
                                                View Details
                                            </Link>
                                            </div>
                                        </div>
                                        ...
                                        <Link href="/exchanges?status=pending" className="text-base font-bold text-blue-600 underline block text-center mt-2 focus-visible:ring-2 focus-visible:ring-offset-1 focus-visible:ring-black">
                                    ...
                                    <div className="text-center py-4 border-2 border-dashed border-gray-300 rounded-lg">
                                        <p className="text-base text-gray-600 leading-normal">No pending exchanges</p>
                                    </div>
                                    )}
                                </div>
                                <div className="border-t-2 border-black pt-4 mt-6 px-6 pb-6">
                                    <div className="flex justify-between items-center mb-3">
                                        <h3 className="font-satoshi text-xl font-bold text-black">Recent Activity</h3>
                                        <Link href="/activity" className="text-base font-bold text-black no-underline hover:underline focus-visible:ring-2 focus-visible:ring-offset-1 focus-visible:ring-black">
                                            View All
                                        </Link>
                                    </div>
                                    ...
                                    <div className="space-y-4">
                                        ...
                                        <div key={activity.id} className="relative pl-5 border-l-2 border-black">
                                            <div className={`absolute -left-[6px] top-1.5 w-2.5 h-2.5 rounded-sm ${activity.status === 'COMPLETED' ? 'bg-green-500' : 'bg-red-500'}`}></div>
                                            <p className="text-xs font-medium text-gray-500 mb-0.5">
                                              {activity.updatedAt ? new Date(activity.updatedAt).toLocaleDateString('en-US', { month: 'short', day: 'numeric', year: 'numeric' }) : ''}
                                            </p>
                                            ...
                                            <p className="text-sm text-gray-700">
                                              {activity.providerService?.title || 'Service'} · {activity.hours || 0}h
                                            </p>
                                        </div>
                                        ))}
                                    </div>
                                    ...
                                    <div className="min-h-[100px] flex items-center justify-center bg-gray-100 border-2 border-black rounded-lg">
                                        <p className="text-base font-medium text-black leading-normal">No recent activity to display</p>
                                    </div>
                                    )}
                                </div>
                            </div>
                    ...
                    <div className="lg:col-span-3 space-y-8">
                        <div className="bg-white border-2 border-black shadow-[6px_6px_0px_0px_rgba(0,0,0,1)] rounded-lg">
                            <div className="bg-blue-500 px-4 py-3 border-b-2 border-black rounded-t-lg">
                                <h2 className="font-satoshi tracking-tight text-lg font-bold flex items-center text-white">
                                    <User className="mr-2" size={18} strokeWidth={2} /> Profile Setup
                                </h2>
                            </div>
                                ...
                                <div className="space-y-2 text-sm mb-2">
                                    ...
                                    <ul className="space-y-3">
                                        <li className="flex items-center">
                                            ...
                                            <Link href="/profile/edit#radius" className="ml-2 text-black font-medium no-underline hover:underline transition-colors transition-all focus-visible:ring-2 focus-visible:ring-offset-1 focus-visible:ring-black">Set your service radius</Link>
                                        </li>
                                        <li className="flex items-center">
                                            <div className="flex-shrink-0 w-5 h-5 bg-white border-2 border-black flex items-center justify-center rounded-lg">
                                            {!hasMissingSkills && <CheckCircle size={12} className="text-green-600" strokeWidth={2} />}
                                            </div>
                                            <Link href="/profile/edit#skills" className="ml-2 text-black font-medium no-underline hover:underline transition-colors transition-all focus-visible:ring-2 focus-visible:ring-offset-1 focus-visible:ring-black">Add your skills</Link>
                                        </li>
                                        <li className="flex items-center">
                                            <div className="flex-shrink-0 w-5 h-5 bg-white border-2 border-black flex items-center justify-center rounded-lg">
                                            {!hasMissingServices && <CheckCircle size={12} className="text-green-600" strokeWidth={2} />}
                                            </div>
                                            <Link href="/services/new" className="ml-2 text-black font-medium no-underline hover:underline transition-colors transition-all focus-visible:ring-2 focus-visible:ring-offset-1 focus-visible:ring-black">Create your first service</Link>
                                        </li>
                                    </ul>
                                </div>
                        ...
                        <div className="bg-white border-2 border-black shadow-[6px_6px_0px_0px_rgba(0,0,0,1)] rounded-lg">
                            <div className="bg-blue-500 px-4 py-3 border-b-2 border-black rounded-t-lg">
                            <h2 className="font-satoshi tracking-tight text-lg font-bold flex items-center text-white">
                                <Star className="mr-2" size={18} strokeWidth={2} /> Build Reputation
                            </h2>
                            </div>
                        ...
                        <div className="bg-white border-2 border-black shadow-[6px_6px_0px_0px_rgba(0,0,0,1)] rounded-lg">
                            <div className="bg-blue-500 px-4 py-3 border-b-2 border-black rounded-t-lg">
                                <h2 className="font-satoshi tracking-tight text-lg font-bold flex items-center text-white">
                                    <CalendarIcon className="mr-2" size={18} strokeWidth={2} /> Calendar
                                </h2>
                            </div>
                            <div className="p-4 rounded-b-lg">
                                <Calendar
                                    ...
                                    modifiersClassNames={{
                                      scheduled: "bg-yellow-200 border-2 border-yellow-500 relative",
                                    }}
                                />
                                {selectedDate && (
                                    <div className="mt-3 p-3 bg-blue-50 border-2 border-black rounded-lg">
                                        {(() => {
                                            ...
                                            if (ex) {
                                                ...
                                                const serviceTitle = ex.providerService?.title || ex.requesterService?.title || 'Service';
                                                const scheduledTime = ex.scheduledDate ? new Date(ex.scheduledDate).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' }) : '';
                                                return (
                                                    <>
                                                        <h3 className="font-satoshi font-bold text-base mb-1">Scheduled Exchange</h3>
                                                        <p className="text-base text-gray-700 font-bold">{serviceTitle}</p>
                                                        <p className="text-base text-gray-700">{ex.scheduledDate ? new Date(ex.scheduledDate).toLocaleDateString('en-US', { month: 'short', day: 'numeric', year: 'numeric' }) : ''} {scheduledTime && `at ${scheduledTime}`}</p>
                                                        {isRequester && <p className="text-sm text-blue-700 font-semibold mt-1">This service is due for you</p>}
                                                    </>
                                                );
                                            }
                                            return (
                                                <>
                                                    <h3 className="font-satoshi font-bold text-base mb-1">Selected</h3>
