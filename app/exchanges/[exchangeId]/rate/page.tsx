...
const RateExchangePage = () => {
  ...
  const canRate = React.useMemo(() => {
    ...
    const isInvolved = userId === exchange.providerId || userId === exchange.requesterId;
    const isCompleted = exchange.status === 'COMPLETED';
    
    // Check if user has already rated
    const hasProviderRated = userId === exchange.providerId && exchange.requesterRating !== null;
    const hasRequesterRated = userId === exchange.requesterId && exchange.providerRating !== null;
    
    return isInvolved && isCompleted && !(hasProviderRated || hasRequesterRated);
  }, [exchange, session]);
    ...
    return (
      <div className="min-h-screen bg-blue-50 p-4">
        <div className="max-w-md mx-auto bg-white p-6 border-2 border-black shadow-[6px_6px_0px_0px_rgba(0,0,0,1)]">
          ...
          <p className="mb-4">The exchange you're looking for doesn't exist or you don't have permission to view it.</p>
    ...
    return (
      <div className="min-h-screen bg-blue-50 p-4">
        <div className="max-w-md mx-auto bg-white p-6 border-2 border-black shadow-[6px_6px_0px_0px_rgba(0,0,0,1)]">
          ...
          <p className="mb-4">You've already rated this exchange or it hasn't been completed yet.</p>
          <Link href={`/exchanges/${exchangeId}`} className="text-blue-600 hover:underline flex items-center">
      ...
      <div className="max-w-md mx-auto">
        ...
        <div className="bg-white p-6 border-2 border-black shadow-[6px_6px_0px_0px_rgba(0,0,0,1)]">
          ...
          <form onSubmit={handleSubmit}>
            <div className="mb-6">
              <p className="mb-2 font-medium">How would you rate this exchange?</p>
              <div className="flex items-center">
                {[1, 2, 3, 4, 5].map((value) => (
                  ...
                  >
                    <Star
                      size={32}
                      fill={(hoveredRating || rating) >= value ? "#facc15" : "none"}
                      stroke={(hoveredRating || rating) >= value ? "#facc15" : "#64748b"}
