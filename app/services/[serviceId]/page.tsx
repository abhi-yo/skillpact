...
import { 
    Loader2, AlertCircle, Star, UserCircle, MapPin, Tag, CalendarDays, ArrowLeft, CheckCircle 
} from 'lucide-react';
...
const ServiceDetailPage: React.FC = () => {
    ...
    const { data: service, isLoading, error } = trpc.service.getServiceById.useQuery(
        { id: serviceId },
        { enabled: !!serviceId } // Only run query if serviceId is available
    );

    // Mutation for creating an exchange request
    const createExchangeMutation = trpc.exchange.createExchangeRequest.useMutation({
        onMutate: () => {
            ...
            <div className="max-w-4xl mx-auto">
                ...
                <Card className="border-2 border-black rounded-lg shadow-[6px_6px_0px_0px_rgba(0,0,0,1)] bg-white py-0">
                       ...
                       <div className="flex items-center space-x-3 p-3">
                         {service.user?.image ? (
                            <Image 
                             ...
                             <UserCircle size={48} className="border-2 border-black rounded-full p-1 text-gray-500"/>
                         )}
                         <div>
                            <p className="text-xs text-gray-600">Provided by</p>
                            <p className="font-satoshi text-xl font-bold leading-none">{service.user?.name || 'SkillSwap User'}</p>
                            {service.user?.averageRating !== null && service.user?.averageRating !== undefined && (
                                <div className="flex items-center text-sm text-gray-700 mt-0.5">
                                    <Star size={14} className="mr-1 text-yellow-500 fill-yellow-400"/> 
                                    {service.user.averageRating.toFixed(1)} 
                                    <span className="ml-1 text-xs">({service.user?.ratingCount || 0} reviews)</span>
                                </div>
                            )}
                         </div>
                       </div>
                    ...
                    <CardContent className="px-4 pb-4 pt-2">
                        <CardTitle className="font-satoshi tracking-tight text-2xl font-bold mb-1 leading-none">{service.title}</CardTitle>
                        ...
                        <p className="text-base text-gray-800 whitespace-pre-wrap mb-2">{service.description || 'No description provided.'}</p>
                        
                        {/* Rate/Hours */}
                        <div className="mb-3 p-2 border-2 border-black bg-orange-50">
                            <p className="text-sm font-medium">
                                Rate / Estimated Hours: <span className="font-bold text-base">{service.hourlyRate ?? 'N/A'}</span>
                            </p>
                        </div>
